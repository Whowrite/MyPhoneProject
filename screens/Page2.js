...
const fetchComedy = async () => {
  const response = await fetch('https://my-json-server.typicode.com/Whowrite/MyPhoneProject/comedy');
  if (!response.ok) {
...
export default function Page2({navigation}) {
  useEffect(() => {
    alert("Вітаю на сторінці Комедія");
  }, []);
  const { data: comedy, isLoading, error } = useQuery({
    queryKey: ['comedy'],
    queryFn: fetchComedy,
});
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.text}>
              ...
              <TouchableOpacity
                            onPress={() => {
                              navigation.navigate('InfoAboutAnime', {title: item.title, img: item.image, page: 'Page2', desc: item.description, click: Clicks});
                              setClicks(Clicks + 1);
                            }} style={styles.button}>
        <Image 
